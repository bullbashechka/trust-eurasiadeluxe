...
for folder in ('gallery', 'archives'):
    (ROOT / folder).mkdir(exist_ok=True)
manifest = json.loads((ROOT / 'blender' / 'scene_manifest.json').read_text())
...
assert json.loads((ROOT/'docs'/'validation.json').read_text())['passed']
...
contact([(ROOT/'renders'/name,label) for name,label in shots],'gallery/gallery.jpg',3,520)
route_files = sorted((ROOT/'walkthrough'/'preview').glob('*.png'))
assert len(route_files) == len(manifest['route'])
contact([(p,f'{i+1:02d}. {m["label"]}') for i,(p,m) in enumerate(zip(route_files,manifest['route']))],'walkthrough/contact.jpg',4,400)
cards = '\n'.join(
...
route_cards = '\n'.join(
    f'<figure><a href="../walkthrough/preview/{p.name}"><img src="../walkthrough/preview/{p.name}" alt="{html.escape(m["label"])}" loading="lazy"></a><figcaption>{i+1:02d} · {html.escape(m["label"])}<small>Кадр {m["frame"]} · {(m["frame"]-1)/30:.1f} с</small></figcaption></figure>'
...
<main><header><div class="brand">EURASIA DE LUXE · ТЁПЛЫЙ СОВРЕМЕННЫЙ</div><h1>Квартира 39,87 м²</h1><p>Демонстрационный макет по исходному плану. Потолки 3 м. Все 11 ракурсов — одна сцена с кремовыми стенами, светлым дубом и бронзовыми деталями.</p><nav><a href="../blender/apartment-39-87.blend">Сцена Blender</a><a href="../docs/BLENDER.md">Инструкция</a><a href="../plans/source-plan.png">Исходный план</a><a href="gallery.jpg">Все ракурсы одним листом</a><a href="#route">Маршрут камеры</a></nav></header>
...
(ROOT/'gallery'/'gallery.html').write_text(page.replace('ROUTE_CARDS',route_cards).replace('CARDS',cards))
# Preserve the apartment layout in the portable package; exclude older archives,
# local logs, backups, previews and generated photorealistic variations.
files = [ROOT / 'README.md']
for folder in ('plans', 'blender', 'docs', 'gallery'):
    files += sorted(p for p in (ROOT / folder).iterdir()
                    if p.is_file() and p.suffix in {'.png', '.py', '.md', '.json', '.html', '.jpg', '.blend'})
files += sorted((ROOT / 'renders').glob('*.png'))
files += [ROOT / 'walkthrough' / 'contact.jpg']
files += route_files
checksums = {str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest() for p in files}
checksum_path = ROOT / 'archives' / 'checksums.json'
checksum_path.write_text(json.dumps(checksums, indent=2, ensure_ascii=False))
files.append(checksum_path)
archive = ROOT / 'archives' / 'apartment-39-87-package.zip'
with zipfile.ZipFile(archive, 'w', zipfile.ZIP_DEFLATED, compresslevel=6) as z:
    for p in files:
        z.write(p, Path('39-87') / p.relative_to(ROOT))
